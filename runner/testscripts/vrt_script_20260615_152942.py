...
def capture(context, html_path: Path, out_path: Path):
    """Render one HTML page at a fixed viewport and save a screenshot.
    Reuses a single browser context (one page per render) for speed/reliability."""
    page = context.new_page()
    page.goto(html_path.as_uri())
    page.wait_for_load_state("load")
    page.wait_for_timeout(300)  # settle fonts/layout
...
def main():
    ...
    print(f"Rendering baseline + modified pages for: {SCENARIO} ...", flush=True)
    with sync_playwright() as pw:
        browser = pw.chromium.launch(args=["--no-sandbox"])
        context = browser.new_context(viewport=VIEWPORT, device_scale_factor=1)
        capture(context, HERE / "baseline.html", ARTIFACTS / "baseline.png")
        capture(context, HERE / "modified.html", ARTIFACTS / "modified.png")
        browser.close()
