...
type GridScanProps = {
  // Webcam/face-tracking props are accepted for API compatibility but are
  // intentionally not wired up here (avoids the heavy face-api.js dependency).
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  // Mouse parallax tracked on the window (so it works even when the layer is
  // pointer-events: none behind hero content). Snaps back when idle/off-area.
  useEffect(() => {
  ...
  // Live-update uniforms when style props change (no re-init needed).
  useEffect(() => {
